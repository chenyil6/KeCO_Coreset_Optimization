...
class Online_ICL:
    ...
    def run(self):
        ...
        test_dataset = Subset(test_dataset, list(range(5000))) 
        ...
        for class_id in tqdm(range(len(self.all_class_names)),desc="get samples for each class"):
            ...
            data_list = data_list_all[0:10] + data_list_all[30:70]
            if self.args.sample_method == "random":
                selected_samples = data_list[0:self.args.M // len(self.all_class_names)]
                remaining_samples = data_list[self.args.M // len(self.all_class_names):50]
                ...
                while len(s) < self.args.M // len(self.all_class_names):
                    ...
                    for sample in data_list:
                        if sample in s:
                            continue
                        # 计算样本与已选择样本集合 s 中最近样本的距离
                        min_distance = float('inf')
                        for selected in s:
                            feature1 = self.features_data_train[sample["id"]]
                            feature2 = self.features_data_train[selected["id"]]
    ...
    def run(self):
        ...
        if self.args.bank == "initial":
            ...
            for class_id in tqdm(range(len(self.all_class_names)),desc="get samples for each class"):
                ...
                data_list = all_data_list[0:10]+all_data_list[30:70]
...
class Offline_ICL:
    def __init__(self, args, tokenizer, model, image_processor,device='cuda',processor=None):
        self.args = args
        self.tokenizer = tokenizer
        self.model = model
        self.image_processor = image_processor
    ...
    def store_bank(self, output_pkl_path):
        ...
        for i in tqdm(range(0, len(self.retriever.demonstrations)), desc="Store bank..."):
            ...
            if label not in sample_list:
                sample_list[label] = [sample.feature.cpu()]
            else:
                sample_list[label].append(sample.feature.cpu())
