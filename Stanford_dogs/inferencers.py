...
import torch
from stanford_dataset import StanfordDogDataset
import os
from PIL import Image
from classification_utils import STANFORD_DOG_CLASSNAMES,STANFORD_DOG_ID_TO_LABEL
...
class Online_ICL:
    ...
    def __init__(self, args, tokenizer=None, model=None, image_processor=None,device=None,processor=None):
        ...
        self.all_class_names = STANFORD_DOG_CLASSNAMES
    ...
    def run(self):
        ...
        train_dataset = StanfordDogDataset(
            root="/path/to/stanford-dog-dataset",
            train=True,
            index_file='./stanfordDog_class_indices.pkl'
        )
        ...
        for class_id in tqdm(range(len(self.all_class_names)),desc="get samples for each class"):
            data_list_all = train_dataset.get_data_list_by_class(class_id=class_id)

            data_list = data_list_all[0:self.args.M // len(self.all_class_names)]+data_list_all[self.args.M // len(self.all_class_names):self.args.M // len(self.all_class_names)+self.args.stream // len(self.all_class_names)] 
            
            if self.args.sample_method == "random":
                selected_samples = data_list[0:self.args.M // len(self.all_class_names)]
                remaining_samples = data_list[self.args.M // len(self.all_class_names):self.args.M // len(self.all_class_names)+self.args.stream // len(self.all_class_names)]
        ...
        if self.args.sample_method == "k_center_greedy": 
            with open("./sample/k_center_greedy_support_set.json", 'r') as f:
                idx_list = json.load(f)
            for idx in idx_list:
                support_sample = train_dataset[idx]
                processed_s = self.preprocess_train(support_sample)
            ...
            with open("./sample/k_center_greedy_sample_pool.json", 'r') as f:
                pool_idx_list = json.load(f)
            sample_pool = [train_dataset[idx] for idx in pool_idx_list]
        ...
        self.test_sample_num = 0
        self.right_sample_num = 0
        print("len of M:",len(self.retriever.demonstrations))
...
class FewShot:
    ...
    def run(self):
        ...
        if self.args.bank == "initial":
            for class_id in tqdm(range(len(self.all_class_names)),desc="get samples for each class"):
                data_list_all = train_dataset.get_data_list_by_class(class_id=class_id)
                data_list = data_list_all[0:10] + data_list_all[30:30+self.args.stream // len(self.all_class_names)]
                if self.args.sample_method == "random":
                    selected_samples = data_list[0:self.args.M // len(self.all_class_names)]
                    #remaining_samples = data_list[5:25]
                elif self.args.sample_method == "k_center_greedy":
                    break
                elif self.args.sample_method == "infoscore":
                    break
            ...
            for class_id in tqdm(range(len(self.all_class_names)),desc="get samples for each class"):
                all_data_list = train_dataset.get_data_list_by_class(class_id=class_id)
                #data_list = all_data_list[0:10]+all_data_list[30:30+self.args.stream // len(self.all_class_names)]
                data_list = all_data_list[0:self.args.M // len(self.all_class_names)]+all_data_list[self.args.M // len(self.all_class_names):self.args.M // len(self.all_class_names)+self.args.stream // len(self.all_class_names)] 
            
                for data in data_list:
                    processed_s = self.preprocess_train(data)
                    self.retriever.demonstrations.append(processed_s)
    
        print("验证 coreset的大小:",len(self.retriever.demonstrations))
        ...
        for idx in tqdm(shuffled_indices, desc=f"Inference Stanford_Dogs..."):
            self.inference(test_dataset[idx])
