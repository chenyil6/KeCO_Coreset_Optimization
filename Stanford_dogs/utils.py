import torch
from classification_utils import STANFORD_DOG_ID_TO_LABEL,STANFORD_DOG_CLASSNAMES
import math
import numpy as np
import logging
...
def get_topk_classifications(outputs, classnames_tokens, topk=2, temperature=2.0):
    ...
    predicted_classnames, predicted_probs = get_predicted_classname(
        overall_log_probs,
        k=topk,
        class_id_to_name=STANFORD_DOG_ID_TO_LABEL,
    )
    return predicted_classnames, predicted_probs, overall_log_probs

import torch

def get_topk_classifications_batch(outputs, classnames_tokens, topk, class_id_to_name=STANFORD_DOG_ID_TO_LABEL):
    """
