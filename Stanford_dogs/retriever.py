...
class DynamicReteiever:
    ...
    def update_online(self,query_sample):
        ...
        if self.args.target_select == "random":
            ...
            query_embedding = query_sample.feature.to(device)  


            embeddings = torch.stack([s.feature for s in sample_list]) 
            embeddings = embeddings.to(device)

       
            similarities = F.cosine_similarity(embeddings, query_embedding.unsqueeze(0), dim=-1)  
    ...
    def update_offline(self,):
        self.offline_based_on_batch()

    def offline_based_on_batch(self,):
        device_set = "cuda:" + str(self.args.device)
        device = torch.device(device_set)
            ...
            for batch in batches:
                ...
                for label, dic in m2p.items():
                    ...
                    for idx,p_list in dic.items():
                        ...
                        self.label2sample[label][idx].feature = self.label2sample[label][idx].feature-0.2*gradient
